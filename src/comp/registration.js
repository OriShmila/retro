...
class Registration extends Component {
  ...
  answer() {
    if (this.state.isSuccess === true) {
        ...
        <label>
          <h5>
            <p>
              ...
              לצערינו אינך זכאי/ת להחזר כרגע
              <br />
              תוכל לנסות שוב בשנה הבאה
            </p>
          </h5>
          <Button
            autoFocus
      ...
      return (
        <label>
          <h5>
            <p>מצטערים יש תקלה במערכת אנא נסה מאוחר יותר</p>
          </h5>
          <Button
            autoFocus
            variant="light"
    ...
    if (typeof fields["id"] !== "undefined") {
      ...
      if (this.state.fields["id"].length > 4) {
        ...
        for (let i = this.state.fields["id"].length - 1; i >= 0; i--) {
          let temp =
            j % 2 === 0
              ? 1 * this.state.fields["id"][i]
              : 2 * this.state.fields["id"][i];
          result += temp % 10;
  ...
  handleChange(field, event) {
    ...
    if (field === "id") {
      if (
        !(copy.fields[field] === "" || /^[0-9\b]+$/.test(copy.fields[field]))
      ) {
        return;
      }
    ...
    return (
      <div>
        ...
        <input
          ...
          min="1"
          max="30"
          value={this.state.fields["numBoy"]}
          required
          onChange={this.handleChange.bind(this, "numBoy")}
        ></input>
        &nbsp;&nbsp;
        <label>
          <b>האם כולם משותפים?</b>
        </label>
        &nbsp;
        <select
          className="ifShare"
          onChange={this.handleChange.bind(this, "isShare")}
          required
      ...
      <div className="page">
        {this.state.isReg
          ? ((document.body.style.overflow = "hidden"),
            (
              <Alert key="answer">
                <div className="ans1">{this.answer()}</div>
              </Alert>
            ))
            ...
            <div className="inDetail">
              <Form className="in" onSubmit={this.handleSubmit.bind(this)}>
                <fieldset disabled={this.state.isReg}>
                  ...
                  ></input>
                  <br />
                  {!this.state.temp.isLeagal ? (
                    <span className="eror">{this.errors["id"]}</span>
                  ) : null}
                  <br />
                  <label>
                    <b>מייל:</b>
                  ...
                  <input
                    ...
                    placeholder="exsmple@exsmple.com"
                    onInvalid={(e) =>
                      e.target.setCustomValidity("יש להכניס כתובת מייל חוקית")
                    }
                  ...
                  <select
                    className="isBoy"
                    onChange={this.handleChange.bind(this, "isBoy")}
                    required
                  ...
                  {this.state.temp["isBoy"] === "yes" ? this.boysNum() : null}
                  <div className="contract"></div>
                  <br />
                  <Form.Check type="checkbox">
                    <Form.Check.Input
                      disabled={!this.state.temp.didReadContract ? true : false}
                      required
                    />
                    <Form.Check.Label>
                      אני מאשר שקראתי והסכמתי לתנאי&nbsp;
                      <Link
                        onClick={this.handel.bind(this, "didRead")}
                        to="./retroPlusContract"
                      >
                        חוזה העסקה
                      </Link>
                      <br />
                      {!this.state.temp.didReadContract ? (
                        <span className="eror">{this.errors["didAgree"]}</span>
                      ) : null}
                    </Form.Check.Label>
                  </Form.Check>
                  <div>
